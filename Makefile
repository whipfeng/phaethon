...
darwin-amd64:
	...
	GOOS=darwin GOARCH=amd64 go build $(LDFLAGS) -o $(DIST_DIR)/darwin-amd64/$(BINARY) $(SRC)

darwin-arm64:
	@echo "=== Building macOS arm64 ($(GIT_TAG)) ==="
	GOOS=darwin GOARCH=arm64 go build $(LDFLAGS) -o $(DIST_DIR)/darwin-arm64/$(BINARY) $(SRC)
