...
sequenceDiagram
    ...
    alt 规则匹配到 DIRECT
        Engine->>RealDNS: 解析真实 IP (通过原始默认接口)
...
`ResolveRouteAware()` 通过 `BindContext` 绑定到原始默认接口（TUN 启动时捕获），使用原始上游 DNS 服务器解析，
绕过 TUN DNS 劫持。
...
- [x] 实现 `resolveWithServers()` 并发查询（绑定原始默认接口）
...
- [x] 无 DNS 查询死循环（Socket 绑定到原始默认接口）
