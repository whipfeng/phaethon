...
**代理组健康检查（`/api/groups/{name}/test`）：**

| 场景 | 订阅节点检查方式 | 手动代理检查方式 |
|------|------------------|------------------|
| 组未配置 `health-check-url` | TCP 连接到节点 `server:port`，仅验证端口可达 | 使用该节点自身的 `health-check-url`（若有），否则 TCP only |
| 组配置了 `health-check-url` | 通过节点代理连接到该 URL，验证代理功能正常 | 使用该节点自身的 `health-check-url`（若有），否则 TCP only |

代理组与单代理行为一致：留空 = TCP 端口检查，配置 URL = 深度功能检查。
